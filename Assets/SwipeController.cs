...
{
    bool isDraggin, isMobilePlatform;
    ...
    void CalculateSwipe()
    {
        ...
        if (swipeDelta.magnitude > minswipedelta)
        {
            if (SwipeEvent != null)
            {
                if (Mathf.Abs(swipeDelta.x) > Mathf.Abs(swipeDelta.y)) { SwipeEvent(swipeDelta.x < 0 ? SwipeType.LEFT : SwipeType.RIGHT); }
                else { SwipeEvent(swipeDelta.y > 0 ? SwipeType.UP : SwipeType.DOWN); }
            }
            ResetSwipe();
        }
